...
func GenUniqueId() string {
	...
	if err != nil {
		global.LOGGER.Error("uuid.NewRandom() returned an error: " + err.Error())
	}
	return id.String()
}
...
func publish(sub *subscription, packet *packets.PublishPacket) {
	...
	switch packet.Qos {
	case QosAtMostOnce:
		if sub.client.typ == LOCAL {
			err := sub.client.WriterPacket(packet)
			if err != nil {
				global.LOGGER.Error("process message for psub error,  ", zap.Error(err))
			}
		} else {
			err := BROKER.router.Forward(sub.client.info.NodeID, &model.Forward{
				ClientID:  sub.client.info.ID,
				Packet:    packet,
				Timestamp: time.Now().Unix(),
			})
			if err != nil {
				global.LOGGER.Error("forward message error,  ", zap.Error(err))
			}
		}
	...
	default:
		global.LOGGER.Error("publish with unknown qos", zap.String("ClientID", sub.client.info.ID))
		return
	}
}

func publishFoward(client *client, packet *packets.PublishPacket) {

	switch packet.Qos {
	case QosAtMostOnce:
		err := client.WriterPacket(packet)
		if err != nil {
			global.LOGGER.Error("process message for psub error,  ", zap.Error(err))
		}
	case QosAtLeastOnce, QosExactlyOnce:
		client.inflightMu.Lock()
		client.inflight[packet.MessageID] = &inflightElem{status: Publish, packet: packet, timestamp: time.Now().Unix()}
		client.inflightMu.Unlock()
		err := client.WriterPacket(packet)
		if err != nil {
			global.LOGGER.Error("process message for psub error,  ", zap.Error(err))
		}
		client.ensureRetryTimer()
	default:
		global.LOGGER.Error("publish with unknown qos", zap.String("ClientID", client.info.ID))
		return
	}
}
